...
% Choose method: draw or enter parameters
method_choice = questdlg('Choose method:', 'Method Selection', 'Draw Ellipse', 'Enter Parameters', 'Draw Ellipse');

% Display the Hartmanngram
figure(1);
...
if strcmp(method_choice, 'Draw Ellipse')
    % Draw an ellipse
    fig = uifigure;
    fig.Position = [1396 105 480 170]; % Create a centered smaller mask
    
    h = drawellipse('Center', [N21, N21], 'SemiAxes', [0.5*N21, 0.5*N21], 'StripeColor', 'r');
    
    % Create a text annotation showing the current parameters (will be updated)
    param_box = annotation('textbox', [0.02, 0.02, 0.3, 0.15], 'String', '', 'FitBoxToText', 'on', 'BackgroundColor', 'white');
    
    % Add a listener for position changes (correct event names for ROI)
    addlistener(h, 'MovingROI', @(src, evt) updateEllipseParams(h, param_box));
    addlistener(h, 'ROIClicked', @(src, evt) updateEllipseParams(h, param_box));
    
    % Initial update
    updateEllipseParams(h, param_box);
    
    uialert(fig, 'Reshape, rotate, move ellipse, then click here', 'Program Information', 'Icon', 'info', 'CloseFcn', 'uiresume(fig)');
    uiwait(fig);
    
    % Get final parameters
    final_center = h.Center;
    final_semiaxes = h.SemiAxes;
    final_rotation = h.RotationAngle;
    
    % Print final parameters
    fprintf('\nFinal Ellipse Parameters:\n');
    fprintf('  Center: [%.2f, %.2f]\n', final_center(1), final_center(2));
    fprintf('  SemiAxes: [%.2f, %.2f]\n', final_semiaxes(1), final_semiaxes(2));
    fprintf('  Rotation: %.2f degrees\n', final_rotation);
    
    close(fig);
    
else % Enter Parameters
    % Create input dialog for parameters
    prompt = {'Center X:', 'Center Y:', 'Semi-Major Axis:', 'Semi-Minor Axis:', 'Rotation Angle (degrees):'};
    dlgtitle = 'Enter Ellipse Parameters';
    dims = [1 40];
    default_input = {num2str(N21), num2str(N21), num2str(0.5*N21), num2str(0.5*N21), '0'};
    answer = inputdlg(prompt, dlgtitle, dims, default_input);
    
    if ~isempty(answer)
        center_x = str2double(answer{1});
        center_y = str2double(answer{2});
        semi_major = str2double(answer{3});
        semi_minor = str2double(answer{4});
        rotation = str2double(answer{5});
        
        % Draw the ellipse with specified parameters
        h = drawellipse('Center', [center_x, center_y], 'SemiAxes', [semi_major, semi_minor], ...
             'RotationAngle', rotation, 'StripeColor', 'r', 'InteractionsAllowed', 'none');
        
        % Display parameters on the figure
        param_text = {['Ellipse Parameters:'],...
                      ['Center: [' num2str(center_x, '%.2f') ', ' num2str(center_y, '%.2f') ']'],...
                      ['SemiAxes: [' num2str(semi_major, '%.2f') ', ' num2str(semi_minor, '%.2f') ']'],...
                      ['Rotation: ' num2str(rotation, '%.2f') ' degrees']};
        annotation('textbox', [0.02, 0.02, 0.3, 0.15], 'String', param_text, 'FitBoxToText', 'on', 'BackgroundColor', 'white');
    end
end
...
%%%Before using the Zernike coeff, calculates teh defocus term.

% Set NaN values where the mask is zero
cr = double(cr);  % Convert from logical to double
m(m == 0) = NaN;  % Now you can assign NaN
...
defocus_term = focdis;
propagation_distance = focdis-1e-3;
[field, intensity] = propagateToFocus(phasef, hfc, wl, propagation_distance, defocus_term, pixel);
...
function [fwhm_x, fwhm_y] = calculateFWHM(intensity, peak_x, peak_y)
    % Get profiles
    x_profile = intensity(peak_y,:);
    y_profile = intensity(:,peak_x);
    
    % Find FWHM
    half_max = 0.5;  % Since intensity is normalized
    
    % X direction
    x_above = find(x_profile > half_max);
    fwhm_x = x_above(end) - x_above(1);
    
    % Y direction
    y_above = find(y_profile > half_max);
    fwhm_y = y_above(end) - y_above(1);
end

% Function to update ellipse parameter display
function updateEllipseParams(h, textbox)
    center = h.Center;
    semiaxes = h.SemiAxes;
    rotation = h.RotationAngle;
    param_text = {['Ellipse Parameters:'],...
                  ['Center: [' num2str(center(1), '%.2f') ', ' num2str(center(2), '%.2f') ']'],...
                  ['SemiAxes: [' num2str(semiaxes(1), '%.2f') ', ' num2str(semiaxes(2), '%.2f') ']'],...
                  ['Rotation: ' num2str(rotation, '%.2f') ' degrees']};
    textbox.String = param_text;
end