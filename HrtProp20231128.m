...
% that the unknown reference wave front is given by the best fit of of ths spots.
% Then use BPM - beam propagation method (3D), based on K. Okamoto Ch 7
% all units are microns
% Masoud, Mabel, Barbara, Erez, 2020-2023 (exact date is in programme title)
...
% %% 1. Parameters
% l     =   199.953 * mm; % distance Hartmann-camera
% pixel = 7.4 * um; % camera pixelsize
% pitch =  250 * um; % Hartmann holes pitch
% wl    = 13.5 * nm; % wave length
% thrsh = 0.05; % noise threshold
% thr   = 0.05; % % mask threshold
% limit = 0;    % noise after smoothing
...
%hr = importdata('C:\Users\ruizlope\Desktop\schwarzschild\WFS_Data_for_Erez\WFS_Data_for_Erez\Reference_DESY_2.txt');
%if isstruct(hr), hr = hr.data; end
...
% h_int = drawellipse ('Center',[N21, N21],'SemiAxes',.2*[N21, N21], 'StripeColor','r'); % fit ellipse to data
% uialert(fig,'Internal ring: Reshape, rotate, click here', 'Program Information','Icon','info','CloseFcn','uiresume(fig)'); uiwait(fig); %circshift is to  move the mask to the center of the figure
hc = round (h.Center); cr = circshift (createMask(h), hc - [N21, N21]); close (fig);  % ellipse containg data
%cr_inte = circshift (createMask(h_int), hc - [N21, N21]);
...
% Usage in your plot:
% colormap(custom_nipy_spectral())

% Assuming 'image' is your original image
 m(m==0)=NaN;
phasezf=phzf.*m;
intensity=hfc.*m;
clipped_image = intensity(512:1532, 512:1532);  % Clip from 512 to 1532 in both dimensions
resized_image = imresize(clipped_image, [2048 2048]);  % Resize to 2048x2048
...
xticks(linspace(0, 2048, 6));
xticklabels(num2str(linspace(0, 15.2, 6)', '%.1f'));
yticks(linspace(0, 2048, 6));
...
%     figure (33); set (gcf, 'position', [1 1 1919 545]);
%    subplot (2,nd,1); imagesc (intensity);  title ([lams '\mum; amp before']); axis image; axis off; colorbar h;
%     subplot (2,nd,nd+1); imagesc (unwrapc (angle(field0))); axis image; title ('phase before'); axis off; colorbar h; drawnow
%      figure (34); set (gcf, 'position', [1 1 1919 245]);
%         subplot (1,nd,1); imagesc (intensity);  title (['amp before']); axis image; axis off; colorbar h;
    for zpixel = z1:z2
        ...
        %if round (zx) == zx
%             figure (33)
%             subplot (2,nd,zx+1); imagesc (intn); axis image; title([zams '\mum amp']); axis off; colorbar h; colormap ("turbo")
%             subplot (2,nd,zx+nd+1); imagesc (unwrapc (angle(field))); axis image; title([zams '\mum phase']); axis off; colorbar h
            %figure (34)
...
[focus_position, focus_size, focus_image] = analyze_focus(phasezf, 10e-6, dis_Hart_source)

function cmap = custom_nipy_spectral()
    % Create a custom colormap similar to nipy_spectral
    cmap = [...
    ...
    % Interpolate to create a smooth 256-color map
